...
/**
 * The primary cell of a row: what the row is, with the one line of context
 * that identifies it. Shopify's lists lead with this shape and so do ours.
 */
export function TitleCell({
  ...
  subtitle?: ReactNode;
  href?: string;
  /** A thumbnail or icon tile, 40px square. */
  media?: ReactNode;
}) {
  const body = (
  ...
  if (!href) return body;
  return (
    // min-h-11 so the row's own name clears the 44px touch minimum in the card
    // layout, where it is the primary way into the record.
    <a href={href} className="focus-ring flex min-h-11 min-w-0 items-center rounded">
      {body}
    </a>
  );
}
