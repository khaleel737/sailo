import { contactResource, orderResource } from "@/lib/api/resources";
import type { Client, Order, OrderItem, Shop } from "@/db/schema";
 ...
 * database, the network or a secret lives in the sibling files, which are
 * server-only.
 *
 * **The objects are not described here.** `lib/api/resources.ts` owns what an
 * order or a contact looks like to the outside world, and the REST API returns
 * the very same shapes — so a consumer that receives `order.paid` and then
 * fetches the order sees identical field names. This file owns only the
 * catalogue and the envelope around it.
 *
