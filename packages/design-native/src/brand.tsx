...
/**
 * The Sailo mark and the Sailo wordmark, as vectors.
 *
 * These are the *same paths* `apps/web/public/brand/sailo-mark.svg` and
 * `sailo-logo.svg` hold — lifted verbatim rather than redrawn, because a logo
 * that is 2% different between the website and the app is the kind of thing
 * nobody can name and everybody notices. If the mark is ever redrawn, both
 * files change together or neither does.
 *
 * Drawn with `react-native-svg`, which is already a peer of this package and a
 * dependency of the app, so this costs no new dependency and no dev-client
 * rebuild. A raster would have been the other option and is the wrong one: the
 * largest PNG in `apps/mobile/assets` is 1024px and it is an *app icon* — a
 * mark with a background plate and safe-area padding baked in, which is not
 * what goes on a welcome screen.
 *
 * `tone` rather than `color`, so the mark obeys the same vocabulary as
 * everything else and turns white on a green ground without a screen picking a
 * hex. There is deliberately no `color` prop.
 */

/**
 * The sail, alone. Square, so it drops into an avatar slot, a header, or the
 * splash without the caller reasoning about aspect ratio.
 */
export type BrandMarkProps = {
  /** Rendered width and height, in points. @default 40 */
  size?: number;
  /** @default "brand" */
  tone?: Tone;
  /**
   * What a screen reader announces. Omit it when a visible "Sailo" sits beside
   * the mark — otherwise the name is read twice.
   */
...
/*
 * The path data, exactly as the SVG files hold it.
 *
 * Kept as module constants rather than inline so the JSX below stays readable,
 * and so a diff that touches the drawing is visibly a diff that touches the
 * drawing.
 */
const MARK_VIEW_BOX = "0 0 64 64";
const MARK_TRANSFORM = "translate(7.05 0.00) scale(0.056869)";
const MARK_PATH =
...
export const BrandMark = memo(function BrandMark({
  ...
  return (
    <Svg
      ...
      /* Silent unless it was given a name — a mark beside the word "Sailo" is
         decoration, and VoiceOver reading "Sailo, Sailo" is the tell. */
      accessibilityElementsHidden={!accessibilityLabel}
      importantForAccessibility={accessibilityLabel ? "yes" : "no-hide-descendants"}
      testID={testID}
    >
      <Path d={MARK_PATH} transform={MARK_TRANSFORM} fill={toneColor(colors, tone)} />
    </Svg>
  );
});

/**
 * The mark and the word, locked up as the brand draws them.
 *
 * `height` rather than `width`, because a wordmark is set to a *cap height* in
 * every layout it appears in — beside a title, in a header — and asking a
 * caller for a width means they have to know the aspect ratio to keep two
 * wordmarks on one screen the same size.
 */
export type WordmarkProps = {
  /** Rendered height, in points. The width follows. @default 28 */
  height?: number;
  /** @default "brand" */
  tone?: Tone;
  testID?: string;
...
const LOGO_VIEW_BOX = "0 0 2264.1 700";
const LOGO_ASPECT = 3.2344;
/* Same drawing as the standalone mark — only the fitting transform differs,
   which is why there is one path constant and two transforms. */
const LOGO_MARK_TRANSFORM = "scale(0.62200)";
const LOGO_TEXT_TRANSFORM = "translate(755.9 154.3)";
const LOGO_TEXT_PATH =
...
export const Wordmark = memo(function Wordmark({
  ...
  return (
    /*
     * Wrapped, and the wrapper carries the accessible name rather than the
     * `Svg` doing it.
     *
     * `react-native-svg` maps `accessibilityLabel` onto the platform view it
     * renders, and on Android that view is a `TextureView` whose label is read
     * inconsistently across TalkBack versions. A `View` with `accessible` set
     * is the one thing both platforms agree about, and the wordmark is the one
     * graphic in the app that genuinely has to be announced — it is the
     * product's name, and on the welcome screen it is the only thing that says
     * what this app is.
     */
    <View
      accessible
    ...
    >
      ...
      >
        <Path d={MARK_PATH} transform={LOGO_MARK_TRANSFORM} fill={fill} />
        <Path d={LOGO_TEXT_PATH} transform={LOGO_TEXT_TRANSFORM} fill={fill} />
      </Svg>
    </View>
  );
});
