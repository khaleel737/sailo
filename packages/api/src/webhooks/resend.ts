/*
 * WHY THE HANDLER IS HERE AND NOT IN A ROUTE
 *
 * It was this file in `apps/web/src/app/api/resend/webhook/route.ts` and, byte
 * for byte, this file again in `apps/api`. The dual mount is deliberate — the
 * delivery URL lives in the Resend dashboard, which only a human can change, so
 * deleting the web copy in the same commit that adds the API one would drop every
 * bounce and complaint in between — but two origins answering one webhook does
 * not require the signature check to exist twice.
 *
 * A duplicated signature verifier is the worst version of this: whichever copy a
 * future fix lands in is the only one that is safe, and the other keeps answering
 * on a URL that is still live in somebody's dashboard.
 *
 * See `docs/api-cutover.md` for the switch order.
 */
