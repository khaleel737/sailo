...
import { findTool, toolResponse, toolsFor } from "./tools";

/*
 * WHY THE HANDLER IS HERE AND NOT IN A ROUTE
 *
 * It was 353 lines inside `apps/web/src/app/api/mcp/route.ts`, and then a second
 * copy of the same 353 lines inside `apps/api/src/app/api/mcp/route.ts`. The
 * dual mount is deliberate and documented — a published endpoint cannot move
 * without a window where it answers nowhere — but nothing about answering on two
 * origins requires the protocol to be implemented twice.
 *
 * This is JSON-RPC framing, version negotiation and tool dispatch: transport,
 * which is what this package is for. What stays in a route file is the two lines
 * that are genuinely Next's — `maxDuration`, and the method exports.
 */
