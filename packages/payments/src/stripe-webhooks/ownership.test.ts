...
import { intentIdOf, sameAccount, sendingAccount } from "./ownership";
 ...
 *
 * These cases live here rather than in apps/web because the rules they cover
 * moved here: the app that imports them is no longer the only one that can,
 * and a seam tested only through one of its two consumers is a seam whose
 * guarantees leave with that consumer.
 */
