 ...
 *
 * Here rather than in `apps/web` because the scanner runs on a phone through
 * `@sailo/api`, and a volunteer's pass has to mean the same thing to both
 * servers. `doorUrl` did not come with it: that builds a link to this app from
 * this app's own `NEXT_PUBLIC_APP_URL`, and it is the one thing in the file
 * that is genuinely the web app's. `apps/web/src/lib/door-pass.ts` keeps it and
 * re-exports the rest.
 */
