...
export async function platformHoldingsFor(
  ...
  return {
    ...
    statementDescriptor: PLATFORM_STATEMENT_DESCRIPTOR,
...
/**
 * Sailo's own statement descriptor.
 *
 ...
 * sellers; this is ours, and it is a constant here rather than a column because
 * there is exactly one platform account and its descriptor is a deployment fact.
 *
 * Applied to the Stripe account by `syncPlatformDescriptor`; this is what the
 * evidence *claims* was shown, so the two must not drift — which is why the
 * setter reads this constant rather than a string of its own.
 */
...
export async function rollUpPlatformUsage(
  ...
  const paid = await db
    ...
    .from(shops)
    .where(and(sql`${shops.plan} <> 'free'`, isNull(shops.deletedAt)));

  let written = 0;

  for (const shop of paid) {
    const [[signinRow], [orderRow], [productRow], [viewRow], [adminRow]] =
      await Promise.all([
        db
          .select({ n: count() })
          .from(accountEvents)
          .where(
            and(
              ...
              eq(accountEvents.kind, "signin"),
              gte(accountEvents.at, start),
              lt(accountEvents.at, end),
            ),
          ),
        db
          .select({ n: count() })
        ...
        db
          .select({ n: sql<string>`coalesce(sum(${visitDaily.visits}), 0)` })
          .from(visitDaily)
          .where(and(eq(visitDaily.shopId, shop.id), eq(visitDaily.day, start))),
        db
          .select({ n: count() })
          .from(accountEvents)
          .where(
    ...
    await db
      ...
      .values({
        shopId: shop.id,
        day: key,
        signins: signinRow?.n ?? 0,
        ordersProcessed: orderRow?.n ?? 0,
        productsActive: productRow?.n ?? 0,
        emailsSent: 0,
        storefrontViews: Number(viewRow?.n ?? 0),
        adminActions: adminRow?.n ?? 0,
        rolledUpAt: new Date(),
      })
      .onConflictDoUpdate({
 ...
 * doing it badly is worse than not doing it: an unqualified enhanced payload is
 * rejected, and the rejection takes the ordinary evidence with it.
 */
