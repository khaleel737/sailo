...
import { normalizeTicketCode } from "./tickets";

/**
 * What the door needs to know, as opposed to what a single scan decides.
 *
 * The check-in decision lives in `./tickets` and is a claim in a WHERE clause.
 * Everything here is a read: the list of events worth standing at, the counters
 * above the door, and the list a volunteer searches when a guest turns up with
 * a dead phone and a name.
 *
 * Here rather than in `apps/web/src/lib/queries` because the scanner is the
 * app's reason to exist on a phone: `events.list` and `events.door` in
 * `@sailo/api` answer with exactly these shapes, and a second implementation
 * would be a counter over the door that disagrees with the one on the laptop
 * beside it. `apps/web/src/lib/queries/tickets.ts` re-exports every name, so
 * the admin screens are untouched — and A10 reads `DoorRow`,
 * `DoorListFilters`, `DOOR_FILTERS`, `DoorStats` and `EventSummary` from here
 * as a read-only consumer, so those five keep their names and their shapes.
 */
