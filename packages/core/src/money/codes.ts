/**
 * Which currencies exist, and how many minor units each one has.
 *
 * The table, and only the table. `format.ts` renders a number with it and
 * `parse.ts` reads one with it; both ask this file rather than assuming two
 * decimal places, which is the assumption that turns ¥1,000 into ¥10.
 *
 * Split out of a 557-line `currency.ts` that held the table, both directions of
 * conversion, and — for no reason anyone could state — a file-size formatter.
 */
