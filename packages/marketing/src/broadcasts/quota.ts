...
/**
 * How many broadcast deliveries the whole platform has sent in the last 24
 * hours.
 *
 * Named for the table it counts, not for what it is used for. There is a second
 * platform-wide ceiling next door in `../lifecycle/state` over `lifecycleEmails`,
 * and while both were called `sentTodayPlatform` in two folders of one app,
 * nothing said they were different — the collision only surfaced when the two
 * folders became one package and the barrel refused to export the name twice.
 * Two ceilings that can be confused for each other is one ceiling checked
 * against the wrong table.
 */
export async function broadcastsSentTodayPlatform(now = new Date()): Promise<number> {
  const since = new Date(now.getTime() - 24 * 3_600_000);
  const [row] = await getDb()
    .select({ n: sql<string>`count(*)` })
    .from(broadcastDeliveries)
...
export async function budgetFor(
  ...
  const [shopSent, platformSent] = await Promise.all([
    ...
    broadcastsSentTodayPlatform(now),
  ]);
