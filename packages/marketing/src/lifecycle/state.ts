 ...
 *
 * Named for the table it counts. `broadcastsSentTodayPlatform` in
 * `../broadcasts/quota` is the same shape over `broadcastDeliveries`, and the
 * two were both called `sentTodayPlatform` until this package merged them.
 */
export async function lifecycleSentTodayPlatform(now = new Date()): Promise<number> {
