/*
 * Mounted on the API origin as well as on apps/web — see `../v1/shop/route.ts`
 * for why a published contract is dual-mounted rather than moved. Same
 * authentication story: an `Authorization` API key, never a session cookie.
 *
 * Both mounts are now four lines over one handler in `@sailo/api/mcp`, so the
 * two origins cannot answer differently.
 */
