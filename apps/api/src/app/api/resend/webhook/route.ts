 ...
 * The delivery URL lives in the Resend dashboard, which is configuration only a
 * human can change. Deleting the web copy in the same commit that adds this one
 * would drop every bounce and complaint notification in the window between the
 * deploy and somebody remembering to update it — and a dropped complaint is a
 * sending reputation nobody is watching.
 *
 * So both answer, the switch happens in the dashboard when you are ready, and
 * `docs/api-cutover.md` records the order. Delivering to both at once would be
 * harmless anyway: the handler is idempotent per event id.
 *
 * Safe to mount twice for the reason `partner/events` is — it authenticates from
 * a Svix signature over the raw body, never from a session cookie.
 * ───────────────────────────────────────────────────────────────────────────
 */
