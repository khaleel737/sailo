...
/**
 * The landing page, in one named language.
 *
 * The locale is a parameter rather than a cookie read, and that is the whole
 * point of this file existing. While the copy was chosen by a cookie the page
 * had to carry `instant = false`: it could never prerender, so the most
 * important URL on the site was the only one paying a server round trip on
 * every visit — and 34 of the 35 languages it was written in had no address a
 * crawler could index. `/` and `/<locale>` both render this, both prerender,
 * and each one declares the other 34 as `hreflang` alternates.
 *
 * `getDictionary` and `directionOf` are the request-free halves of `getT()`,
 * which is what keeps this synchronous with respect to the request.
 */
export function HomeView({ locale }: { locale: Locale }) {
  /*
   * The signed-in redirect is not here — it is in `src/proxy.ts`.
   *
