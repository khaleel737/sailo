...
const admin = (path = "") => `${appUrl()}/admin${path}`;
const shopUrl = (handle: string) => `${appUrl()}/${handle}`;
const demoUrl = () => `${appUrl()}/${HERO_DEMO.handle}`;
...
function noShop1(state: LifecycleState): Draft {
  return {
    ...
    cta: { href: `${appUrl()}/onboarding`, label: "Set up my shop" },
...
function noShop2(state: LifecycleState): Draft {
  return {
    ...
    highlight: demoUrl(),
    cta: { href: `${appUrl()}/onboarding`, label: "Build mine" },
  };
}
