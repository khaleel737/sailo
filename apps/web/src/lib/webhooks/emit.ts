...
import { contactPayload, envelope, orderPayload, type WebhookEvent } from "./events";
 ...
 * Every call site wraps this in `after()`, which runs once the response has
 * gone, and `notify-seller.test.ts` is the pattern for asserting that a call
 * site kept doing so.
 *
