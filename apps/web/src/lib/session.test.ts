...
describe("requireShop, audited", () => {
  ...
  it("is called 151 times, and every one names a real permission", () => {
     ...
     *
     * 149 → 151 is spec 51's roster: `saveStaffMember` and `toggleStaffActive`
     * in `actions/staff.ts`, both `settings:write`. Who takes bookings is
     * shop-wide configuration in the same sense the opening hours are, and
     * `settings` is the resource a manager holds read-only.
     *
