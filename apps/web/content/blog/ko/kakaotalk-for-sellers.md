...
계좌이체와 대금상환처럼 사일로를 거치지 않는 결제 수단에는 사일로 수수료가 없습니다. 카드는 다릅니다. 결제 승인을 받은 Stripe 계정을 연결해야 하고, 상품 금액의 {{fee_range}}%를 가져갑니다. 할인 후 상품 금액 기준이고 배송비와 세금은 빠집니다. 사일로의 카드 레일은 Stripe 전용이라 Stripe가 없는 곳에서는 돈을 더 내도 카드 버튼이 안 생깁니다. 한국에서 Stripe 계정을 열 수 있는지는 이 글을 쓰면서 확인하지 않았으니, stripe.com/global에서 직접 보세요. 국가별 상황은 계속 바뀝니다.
