...
Sailo のカード決済は Stripe に承認された接続済みアカウントが要ります。手数料は商品代の {{fee_range}}% で、これはどのプランでも同じ率です。割引後の商品代にかかり、送料と税は対象外。お金は Sailo を経由せず、あなたの Stripe アカウントに直接入ります。
...
無料プランは $0 で、商品{{free_products}}点まで。分析データは{{free_analytics_days}}日分。チャット系と手動の支払い方法に加えて、カード決済も {{fee_free}}% で使えます。

Pro は ${{pro_monthly}}/月（年払い ${{pro_yearly}}）で商品{{pro_products}}点、分析1年、Sailo のバッジが消えて、CSV書き出しがつきます。カード決済はここにも**ありません**。

Business は ${{business_monthly}}/月（年払い ${{business_yearly}}）で商品数の上限なし、分析3年、カード手数料 {{fee_business}}%、アフィリエイト。
