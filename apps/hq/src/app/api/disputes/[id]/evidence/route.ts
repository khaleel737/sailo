...
/**
 * Attaching a document to a dispute, as staff.
 *
 * A copy of apps/web's route, not a shared one, and it has to be: the uploader
 * component posts to a *relative* `/api/disputes/:id/evidence`, so on
 * hq.sailo.store that URL has to exist on hq.sailo.store. Without this file the
 * staff upload button posted into a 404 — which is exactly the class of bug the
 * split invites and the reason this was checked rather than assumed.
 *
 * The two copies differ in the only way that matters: this one authorises
 * through apps/hq's `dispute-access`, which is staff-only and demands
 * `money:move`; apps/web's is seller-only and demands ownership. One route with
 * an `as` parameter was how this worked before, and the parameter was the weak
 * point — the guard you get depended on a string the caller passed.
 *
...
export async function POST(
  ...
  /*
   * apps/web's copy reads an `as` field off the form here to pick between two
   * guards. This one does not, and the field is ignored: there is a single
   * guard on this origin and it is the staff one. Which authority you get is
   * decided by which deployment you reached, not by anything the caller sends.
   */
